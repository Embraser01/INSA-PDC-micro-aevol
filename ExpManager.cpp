...
void ExpManager::opt_prom_compute_RNA(int indiv_id) {
    ...
    currentOrganism->proteins.clear();
    ...
    for (int prom_idx = 0; prom_idx < currentOrganism->promoters.size(); prom_idx++) {
        ...
        if (currentOrganism->promoters[prom_idx] != nullptr) {
            ...
            cur_pos = cur_pos >= currentOrganism->length() ? cur_pos - currentOrganism->length()
                                                           : cur_pos;
            ...
            if (!no_terminator) {
                ...
                int32_t rna_end =
                        cur_pos + 10 >= currentOrganism->length() ? cur_pos + 10 - currentOrganism->length()
                                                                  : cur_pos + 10;

                int32_t rna_length = 0;

                if (prom_pos > rna_end)
                    rna_length = currentOrganism->length() - prom_pos + rna_end;
                else
                ...
                if (rna_length > 0) {
                    int glob_rna_idx = currentOrganism->rna_count_;
                    currentOrganism->rna_count_ = currentOrganism->rna_count_ + 1;
...
void ExpManager::compute_protein(int indiv_id) {
    ...
    for (int rna_idx = 0; rna_idx < (int) currentOrganism->rna_count_; rna_idx++) {
        if (currentOrganism->rnas[rna_idx]->is_init_)
            resize_to += currentOrganism->rnas[rna_idx]->start_prot.size();
    }

    currentOrganism->proteins.resize(resize_to);
...
void ExpManager::translate_protein(int indiv_id, double w_max) {
    ...
    for (int protein_idx = 0; protein_idx < (int) currentOrganism->protein_count_; protein_idx++) {
        {
            Protein *&protein = currentOrganism->proteins[protein_idx];
            if (protein->is_init_) {
                int c_pos = protein->protein_start, t_pos;
                int end_pos = protein->protein_end;

                c_pos += 13;
                end_pos -= 3;

                c_pos =
                        c_pos >= currentOrganism->length() ? c_pos -
                                                             currentOrganism->length()
                                                           : c_pos;
                end_pos = end_pos < 0 ? currentOrganism->length() + end_pos : end_pos;
                ...
                while (count_loop <
                       ...
                       3 &&
                       codon_idx < 64) {
                    codon_list[codon_idx] = currentOrganism->dna_->codon_at(c_pos);
                ...
                if (nb_m == 0 || nb_w == 0 || nb_h == 0 || protein->w == 0.0 || protein->h == 0.0) {
    ...
    for (int protein_idx = 0; protein_idx < currentOrganism->protein_count_; protein_idx++) {

        if (currentOrganism->proteins[protein_idx]->is_init_) {
            if (lookup.find(currentOrganism->proteins[protein_idx]->protein_start) == lookup.end()) {
                lookup[currentOrganism->proteins[protein_idx]->protein_start] = currentOrganism->proteins[protein_idx];
            } else {
                lookup[currentOrganism->proteins[protein_idx]->protein_start]->e += currentOrganism->proteins[protein_idx]->e;
                currentOrganism->proteins[protein_idx]->is_init_ = false;
            }
        }
